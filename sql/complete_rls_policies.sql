...
-- =====================================================
-- STEP 19: JOB INVITATIONS (used across candidate / employer / MIS flows)
-- =====================================================
...
TO authenticated
USING (
  company_id IN (SELECT company_id FROM employers WHERE user_id = auth.uid())
);
...
TO authenticated
USING (
  EXISTS (SELECT 1 FROM mis_user WHERE user_id = auth.uid())
);
...
WITH CHECK (
  ...
  AND company_id IN (SELECT company_id FROM employers WHERE user_id = auth.uid())
);
...
ON job_invitations FOR UPDATE
TO authenticated
USING (company_id IN (SELECT company_id FROM employers WHERE user_id = auth.uid()))
WITH CHECK (company_id IN (SELECT company_id FROM employers WHERE user_id = auth.uid()));
...
-- =====================================================
-- STEP 20: REFERENCE DATA (public read for dropdowns / filters)
-- =====================================================

ALTER TABLE industries ENABLE ROW LEVEL SECURITY;
ALTER TABLE seniority_levels ENABLE ROW LEVEL SECURITY;
ALTER TABLE job_designations ENABLE ROW LEVEL SECURITY;
...
USING (
  EXISTS (
    ...
    WHERE ji.id = interview_rounds.invitation_id AND c.user_id = auth.uid()
  )
);
...
USING (
  EXISTS (
    ...
    WHERE ji.id = interview_rounds.invitation_id AND e.user_id = auth.uid()
  )
);
...
-- =====================================================
-- STEP 22: LOG TABLES (API uses service role; block anon/authenticated direct access)
-- =====================================================
...
-- MIS may read logs from the dashboard; inserts go through service role only.
...
-- 6. Candidate/employer/MIS registration uses SUPABASE_SERVICE_ROLE_KEY on the server
--    for initial user rows (see createAdminClient); keep that secret off the client.
-- =====================================================
