...
import { landingEase } from '@/lib/motion/landing';
...
export function Header({ showSignIn = true }: { showSignIn?: boolean }) {
    ...
    return (
        <motion.header
            className="sticky top-0 z-50 px-3 pt-3 sm:px-5 lg:px-8"
            initial={{ opacity: 0, y: -16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.45, ease: landingEase }}
        >
            <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3 rounded-2xl border border-border/70 bg-background/70 px-4 py-3 shadow-[0_12px_40px_-16px_rgba(0,0,0,0.35)] backdrop-blur-2xl dark:bg-background/55 dark:shadow-black/50 md:px-5">
                ...
                <nav className="hidden items-center gap-0.5 rounded-full border border-border/40 bg-muted/30 px-1.5 py-1 md:flex dark:bg-muted/20">
                    {navLinks.map((link, i) => (
                        <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ delay: 0.06 + i * 0.05, duration: 0.35, ease: landingEase }}
                        >
                            <Link
                                href={link.href}
                ...
                <div className="flex items-center gap-2">
                    <motion.div
                        ...
                        transition={{ delay: 0.22, duration: 0.35, ease: landingEase }}
                    >
                        <ThemeToggle />
                    </motion.div>
                    {showSignIn && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        transition={{ delay: 0.26, duration: 0.35, ease: landingEase }}
                    >
                        <Button
                            asChild
                            size="sm"
            ...
            <AnimatePresence mode="wait">
                {open && (
                    <motion.div
                        ...
                        className="mx-auto mt-2 max-w-[1400px] overflow-hidden rounded-2xl border border-border/70 bg-background/95 shadow-xl backdrop-blur-xl md:hidden"
                    >
