-- =====================================================
-- COMPLETE RLS POLICIES FOR JOBGENIE
-- =====================================================
-- This file is the single source of truth for ALL Row Level Security
-- policies in the JobGenie application.
--
-- ARCHITECTURE:
--   • Three SECURITY DEFINER helper functions (is_mis_user, is_candidate,
--     is_employer) perform role membership lookups. Because they run as the
--     function owner they bypass RLS on the inner query, preventing the
--     recursive evaluation that caused PostgREST schema cache failures.
--   • All table-level GRANTs are included so PostgREST can expose every
--     table correctly.
--   • (SELECT auth.uid()) forces a single initialisation plan per query,
--     avoiding per-row re-evaluation of the volatile auth function.
--
-- HOW TO USE (after a fresh Prisma migration):
--   1. Run prisma migrate deploy
--   2. Execute this entire file in Supabase SQL Editor or via MCP execute_sql
-- =====================================================
...
-- ============================================================
-- RLS controls row-level access; grants control operation-level access.
-- Both must be satisfied. Without grants PostgREST won't expose the table.
...
-- -----------------------------------------------------------
-- MIS_USER
-- NOTE: "MIS can view all MIS users" previously used a
-- self-referential subquery (mis_user policy querying mis_user),
-- causing PostgREST schema cache ABORT. Fixed using is_mis_user()
-- SECURITY DEFINER which bypasses RLS on the inner lookup.
-- -----------------------------------------------------------
